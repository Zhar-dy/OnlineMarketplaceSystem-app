...
<div class="modal fade" id="CreateShippingModal--{{$payment->id}}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <form action="{{ route('category.store') }}" method="POST">
            @csrf
            <input type="hidden" name="shipping_status" value="{{ $payment->order- }}">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" value="{{$payment->order->product->user->name}}" name="name" id="recipient-name" readonly
                            autocomplete>
                    </div>
                    <div class="mb-3">
                        <label for="message-text" class="col-form-label fw-bold">Buyer Name</label>
                        <input type="text" class="form-control" value="{{$payment->order->user->name}}" name="name" id="recipient-name" readonly
                        autocomplete>
                    </div>
                    <div class="mb-3">
                        <label for="message-text" class="col-form-label fw-bold">Shipping Address</label>
                        <input type="text" class="form-control" value="{{$payment->order->user->address}}" name="name" id="recipient-name" readonly
                        autocomplete>
                    </div>
                </div>
